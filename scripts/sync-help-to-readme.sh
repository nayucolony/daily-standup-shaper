...
Options:
  ...
  --update-sync-help-failure-heading-snapshot Update tests/snapshots/readme-sync-help-failure-heading.md from README Quick check heading line.
  ...
  --all                                 Sync README help options + one-line contract snapshot + test-link line/snapshot + recommended sequence snapshot + sync-help single-line snapshot + summary-line snapshot + sync-help failure-heading/template snapshots + sync-help --help Examples snapshot + sync-help optional-order snapshot.
...
update_sync_help_failure_heading_snapshot=false
...
case "${1:-}" in
  ...
  --update-summary-line-snapshot)
    ...
    ;;
  --update-sync-help-failure-heading-snapshot)
    update_sync_help_failure_heading_snapshot=true
    ;;
  --update-sync-help-failure-template-snapshot)
...
if [ "$update_sync_help_failure_heading_snapshot" = true ]; then
  "$ROOT_DIR/scripts/update-sync-help-failure-heading-snapshot.sh"
fi
