...
## Update Plan (watchdog 2026-02-25 10:10 JST)
反復判定（実行前の直近5サイクル）: `P153 -> P155 -> P156 -> P157 -> P158` で README/sync-help/selfcheck の同系比率は `5/5=1.00`（閾値0.60超過）。
...
## Next
- P160を実施する: `assert_sync_help_all_invariants` の mismatchメッセージ `changed_count=<n>` が changed ラベル数と常に一致することを selfcheck で回帰化する
