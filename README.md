...
# README/スナップショット同期（help/options + one-line contract + links + recommended + sync-line + summary-line + failure-heading/template + help-examples + optional-block + optional-order を1コマンドで揃える）
...
# 対応テスト: [`accepts 0foo (README one-line acceptance)`](./scripts/selfcheck.sh#L1093), [`rejects Foo (README one-line acceptance)`](./scripts/selfcheck.sh#L1094), [`rejects fooA (uppercase suffix, README one-line acceptance)`](./scripts/selfcheck.sh#L1094), [`rejects foo/bar (slash delimiter, README one-line acceptance)`](./scripts/selfcheck.sh#L1094)
...
## Update Plan (watchdog 2026-02-25 10:00 JST)
反復判定（実行前の直近5サイクル）: `P154 -> P153 -> P155 -> P156 -> P157` で README/sync-help/selfcheck の同系比率は `5/5=1.00`（閾値0.60超過）。
...
- [ ] P160: `assert_sync_help_all_invariants` の mismatchメッセージ `changed_count=<n>` が changed ラベル数と常に一致することを selfcheck で回帰化する（Impact: 2, Effort: 2, Evidence: yes）
...
## Next
- P158を実施する: `sync-help-to-readme.sh --all` 実行後の README Quick check 同期対象一覧（12項目）と `assert_sync_help_all_invariants` の検証対象が同時に no diff であることを selfcheckで1ケース回帰化する
